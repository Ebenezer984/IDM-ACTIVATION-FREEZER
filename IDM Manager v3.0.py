print ("this crack is created by ECG®. th3hacker007@gmail.com")
import subprocess
import os
import sys
import time
...
def print_header():
    ...
    clear_screen()
    print(Fore.GREEN + "=" * 70)
    print(Fore.GREEN + "=" + Fore.WHITE + Back.GREEN + " IDM FREEZER & ACTIVATION TOOL ".center(68) + Fore.GREEN + "=")
...
def print_menu():
    ...
    print(Fore.YELLOW + " [7]" + Fore.WHITE + " Visit ECG's Repository")
...
def run_batch_file(parameter="", show_output=True):
    """Run the ECG.cmd batch file with optional parameters."""
    batch_file = Path(__file__).parent / "ECG.cmd"
    ...
    try:
        if parameter:
            ...
            log_message("Executing: ECG.cmd", "INFO")
        ...
        if "introuvable" in str(e) or "not found" in str(e).lower():
            print(Fore.YELLOW + "\nTip: Make sure ECG.cmd is in the same folder as this script.")
        return False
    finally:
        input(Fore.YELLOW + "\nPress Enter to return to the main menu...")
